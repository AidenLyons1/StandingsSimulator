...
export async function GET() {
  try {
    ...
    // Fetch standings from the RapidAPI endpoint
    ...
    if (!standingsResponse.ok) {
      return NextResponse.json(
        { error: `Failed to fetch standings data: ${standingsResponse.status}` },
    ...
    if (!fixturesResponse.ok) {
      return NextResponse.json(
        { error: `Failed to fetch fixtures data: ${fixturesResponse.status}` },
    ...
    console.error('Error in Scottish Championship API route:', error);
    return NextResponse.json(
